...
export default class FastFoods extends Component{
  constructor(props) {
    super(props);
    const store = this.props.navigation.getParam('store', 'NO-ID');
    this.state = {
      defaultIndex: 0,
  ...
  _renderItem = (item, index) => {
    return (
      ...
      >
        <FFCard title={item["title"]} store={this.state.name} storeId={this.state.store_id} token={this.state.token}  navigation={this.props.navigation}/>
      </View>
    );
  };
